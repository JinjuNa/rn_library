...
export default ({ navigation, route, params }) => {
    ...
    const { is_key, access_token } = params;
        ...
        AsyncStorage.getItem('pin')
            .then(result => {
                if (result == null || result == '') {
                    navigation.navigate('PinScreen', { mode: 'create' }); // 신규등록
                }
                else {
                    navigation.navigate('PinScreen', { mode: 'confirm', sid: store.sid, cid: store.cid, access_token: access_token, url: store.url }); // 번호확인
                }
            })
            .catch(error => console.log(error));
    }
    ...
    return (
        <>
            <$Header style={{ backgroundColor: '#111111', height: 80 }} iosBarStyle={"light-content"} backgroundColor={'#111'}>
                ...
                <Left style={{ flex: 1 }}>
                    <Icon name="view-headline" size={30} style={{ fontSize: 30, color: "white" }} onPress={()=>open_drawer()}></Icon>
                </Left>
