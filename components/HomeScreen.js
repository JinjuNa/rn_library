...
function HomeScreen({ navigation, route }) {
  ...
  // 카드결제를 완료하고
  // 팝업창이 닫힐때 새로고침   
  const { pop_id } = ( route.params ) ? route.params:"";  
  useEffect(()=>{
  ...
  return (              
      <Container>
        ...
        { isLogin == 'Y' ? <Logged navigation={navigation} params={{is_key:is_key, access_token: access_token }} /> : null }      
        { isLogin == 'N' ? <Login navigation={navigation} /> : null }              
      </Container>
  );
